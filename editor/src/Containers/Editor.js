...
class Editor extends Component {
  ...
  this.state = {
      ...
      isLoading: true,theme: 'vs-dark', // default theme
  fontSize: 14,
    };
  }
...
increaseFont = (specificSize) => {
  if (specificSize) {
    this.setState({ fontSize: specificSize });
  } else {
    this.setState(prevState => ({
      fontSize: Math.min(prevState.fontSize + 1, 24)
    }));
  }
};

decreaseFont = (specificSize) => {
  if (specificSize) {
    this.setState({ fontSize: specificSize });
  } else {
    this.setState(prevState => ({
      fontSize: Math.max(prevState.fontSize - 1, 10)
    }));
  }
};
handleVideoChat = () => {
